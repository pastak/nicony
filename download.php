...
function getFlv($video_id){
    ...
    //ここで一旦、cookieを更新する
    $ch = curl_seting('http://www.nicovideo.jp/watch/'.$video_id);
    curl_setopt($ch, CURLOPT_COOKIEFILE ,$cookie_file);
    $ch = set_opt( $ch, $cookie_file );
    curl_exec($ch);
    curl_close($ch);
    ...
    $PathVideoDir="./videos";//videoを保存するディレクトリ
    ...
    if(preg_match("/403 Forbidden/",$flvdata)){
        ...
        echo "save".$video_id."\n";
        $xml=simplexml_load_file("http://ext.nicovideo.jp/api/getthumbinfo/".$video_id);
        $title=$xml->thumb->title;
        //file_put_contents($PathVideoDir.'/'.$title." - ".$video_id.'.mp4',$flvdata);
    }
    curl_close($ch);
};
getFlv($argv[1]);
